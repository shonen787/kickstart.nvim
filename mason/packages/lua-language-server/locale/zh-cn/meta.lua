...
cgopt.step          =
[[
执行一次垃圾收集的“步骤”。此选项后可以带一个额外参数 `size`（整数）。
如果 `size` 为正的 n，收集器的行为就像新分配了 n 个字节；如果为 0，则执行一个“基本步骤”。
在增量模式下，基本步骤对应当前的步进大小；在分代模式下，基本步骤会执行一次完整的次要收集，
或在收集器已安排增量步骤时执行该增量步骤。
在增量模式下，如果该步骤完成了一个收集周期，函数返回 `true`；在分代模式下，如果该步骤完成了一个主要收集，返回 `true`。
]]
cgopt.setpause      =
'将 `arg` 设为收集器的 *间歇率* 。'
cgopt.setstepmul    =
'将 `arg` 设为收集器的 *步进倍率* 。'
cgopt.incremental   =
'将收集器模式改为“增量模式”，并返回之前的模式（可能是 `"generational"` 或 `"incremental"`）。'
cgopt.generational  =
'将收集器模式改为“分代模式”，并返回之前的模式（可能是 `"generational"` 或 `"incremental"`）。'
cgopt.isrunning     =
'返回表示收集器是否在工作的布尔值。'
cgopt.param         =
[[
更改和/或检索收集器参数的值。此选项后必须跟一个或两个额外参数：
要更改或检索的参数名称（字符串），以及该参数的可选新值（范围 [0,100000] 的整数）。
调用总是返回该参数的“先前值”；如果未提供新值，则保持不变。Lua 以压缩格式存储这些值，
因此返回的先前值可能不完全等于上次设置的值。
]]
...
collectgarbage      =
[[
该函数是垃圾收集器的通用接口，根据第一个参数 `opt` 执行不同的功能：
• `"collect"`：执行一次完整的垃圾收集循环（默认选项）。
• `"stop"`：停止垃圾收集器的自动执行；之后仅在显式调用时运行，直到 `"restart"`。
• `"restart"`：重启垃圾收集器的自动执行。
• `"count"`：返回 Lua 当前使用的总内存（单位为 K字节，含小数；乘以 1024 得到字节数）。
• `"step"`：执行一次垃圾收集步骤，可选参数 `size` 控制步长；行为与返回值见 `cgopt.step` 的说明。
• `"isrunning"`：返回一个布尔值，指示收集器是否正在运行（即未被停止）。
• `"incremental"`：将模式改为增量模式，并返回之前的模式。
• `"generational"`：将模式改为分代模式，并返回之前的模式。
• `"param"`：更改/检索收集器参数（见各 `gcparam.*` 项），总是返回该参数的先前值。
]]
