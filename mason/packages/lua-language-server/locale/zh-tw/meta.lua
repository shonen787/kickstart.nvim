...
cgopt.step          =
[[
執行一次垃圾回收的「步驟」。此選項後可帶一個額外參數 `size`（整數）。
若 `size` 為正的 n，回收器的行為就像新分配了 n 個位元組；若為 0，則執行一個「基本步驟」。
在增量模式下，基本步驟對應目前的步進大小；在分代模式下，基本步驟會執行一次完整的次要回收，
或在回收器已安排增量步驟時執行該增量步驟。
在增量模式下，若此步驟完成了一個回收週期，函式回傳 `true`；在分代模式下，若此步驟完成了一次主要回收，則回傳 `true`。
]]
cgopt.setpause      =
'將 `arg` 設為回收器的 *間歇率* 。'
cgopt.setstepmul    =
'將 `arg` 設為回收器的 *步進倍率* 。'
cgopt.incremental   =
'將回收器模式改為增量模式，並回傳先前的模式（可能為 `"generational"` 或 `"incremental"`）。'
cgopt.generational  =
'將回收器模式改為分代模式，並回傳先前的模式（可能為 `"generational"` 或 `"incremental"`）。'
cgopt.param         =
[[
更改和／或檢索回收器參數的值。此選項後必須接一或兩個額外參數：
欲更改或檢索的參數名稱（字串），以及該參數的可選新值（介於 [0,100000] 的整數）。
呼叫總是回傳該參數的「先前值」；若未提供新值，則維持不變。Lua 以壓縮格式儲存這些值，
因此回傳的先前值可能不完全等於上次設定的值。
]]
...
collectgarbage      =
[[
垃圾回收器的一般介面；依第一個引數 `opt` 執行不同功能：
• `"collect"`：執行一次完整的垃圾回收（預設）。
• `"stop"`：停止回收器的自動執行；之後僅在顯式呼叫時執行，直到 `"restart"`。
• `"restart"`：重新啟動回收器的自動執行。
• `"count"`：回傳 Lua 目前使用的總記憶體（KBytes，含小數；乘以 1024 得到位元組數）。
• `"step"`：執行一次回收步驟；可選整數 `size` 控制行為與回傳值（參見 `cgopt.step`）。
• `"isrunning"`：回傳布林值，指示回收器是否正在執行（未停止）。
• `"incremental"`：切換為增量模式並回傳先前的模式。
• `"generational"`：切換為分代模式並回傳先前的模式。
• `"param"`：更改／讀取回收器參數（見各 `gcparam.*` 項），總是回傳該參數的先前值。
]]
