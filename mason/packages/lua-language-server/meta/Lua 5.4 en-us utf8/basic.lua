...
---| "step"         # Performs a garbage-collection step. This option may be followed by an integer `size`....If `size` is a positive n, the collector acts as if n new bytes have been allocated; if `size` is zero, the collector performs a basic step....In incremental mode, a basic step corresponds to the current step size; in generational mode, a basic step performs a full minor collection,...or an incremental step if the collector has scheduled one....In incremental mode, the function returns `true` if the step finished a collection cycle; in generational mode, it returns `true` if the step finished a major collection....
---| "isrunning"    # Returns whether the collector is running.
---| "incremental"  # Changes the collector mode to incremental and returns the previous mode (either `"generational"` or `"incremental"`).
---| "generational" # Changes the collector mode to generational and returns the previous mode (either `"generational"` or `"incremental"`).
